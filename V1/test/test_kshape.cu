...
void labeled_process(string file_path,string name)
{
    GpuTimer timer;

    vector<int> label;
    size_t mat_rows,mat_cols;
    ...
    timer.Start();
    k_shape(d_mat, idx, mat_rows, mat_cols, k, iternum);
    timer.Stop();
...
void unlabeled_process(string file_path, const int k, string name)
{
...
void testfunc(string fname, int k, int flag)
{
    cout<<fname<<endl;
    if(flag)
        unlabeled_process(fname, k, "result");
    else
        labeled_process(fname, "result");
}


// ./test -d 0(device name) filePath class_number label_or_unlabel_flag 
//./test -d 0 /home/gpu2/Desktop/srbWorkSpace/tensorCore/proj_1/data/InsectSound/InsectSound_TRAIN 10 1
