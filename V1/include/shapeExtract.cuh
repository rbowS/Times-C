...
__global__ void get_center(T *d_vals, T *d_EigVecs, T *d_centers, int *d_scan_kcount,
                          const size_t k, const size_t MAT_COLS, int *d_c)
{
    
    T finddistance1 = 0;
    T finddistance2 = 0;
    judge_center(d_vals+MAT_COLS*d_c[0], d_EigVecs+(MAT_COLS-1)*MAT_COLS, MAT_COLS, true, finddistance1);
    judge_center(d_vals+MAT_COLS*d_c[0], d_EigVecs+(MAT_COLS-1)*MAT_COLS, MAT_COLS, false, finddistance2);
    if(finddistance1 >= finddistance2)
    {
        ops_center(d_EigVecs+(MAT_COLS-1)*MAT_COLS, MAT_COLS);
    }
        
    memcpy(d_centers+d_c[0]*MAT_COLS, d_EigVecs+(MAT_COLS-1)*MAT_COLS, sizeof(T)*MAT_COLS);
}
                                                                  ...
                                                                  const size_t MAT_COLS)
{
    ...
    for(int i=0; i<k; i++)
    {
        if(k_count[i] != 0)
        {
            ...
            get_center<<<1, 1>>>(d_vals, d_result, d_centers, d_scan_kcount, k, MAT_COLS, d_c);
            cudaDeviceSynchronize();
                   ...
                   const size_t mat_col)
{
    ...
    timer.Stop();
    printf("Data Movement run on GPU: %f msecs.\n", timer.Elapsed());
    ...
    //z_norm_gpu_x(d_mat_val, mat_row, mat_col, 1);
