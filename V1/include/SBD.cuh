...
template<typename T>
__device__ void zeroShift(T *d_vec, T *d_out, const int shift, const size_t vec_size)
{
...
template<typename T>
__device__ void get_maxpos(T *arr, const size_t arr_len, int &maxpos)
{
    T val = -1e10;
...
__global__ void maxAndShift(T *d_mat_ncc, T *d_mat, T *d_mat_shift, 
                            const size_t ncc_len, const size_t mat_row, const size_t mat_col)
{
...
void sbd3D(T *d_mat, T *d_center, T *d_mat_shift, 
          const size_t mat_row, const size_t mat_col)
{
